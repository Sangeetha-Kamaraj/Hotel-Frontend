...
export default function BookingSlots({ setBookingState, BookingState }) {
  ...
  const handleBooking = () => {
    ...
    if (BookingState.selectedSlot) {
      alert(`Successfully booked slot: ${BookingState.selectedSlot}`);
    } else {
        ...
        <Grid container spacing={2}>
          {slots.morning.map((ele) => (
            <Grid item key={ele.time}>
              <Chip
                color={BookingState.selectedSlot === ele.time ? "success" : "default"}
                onClick={() => setBookingState({ ...BookingState, selectedSlot: ele.time })}
                clickable
                label={ele.time}
                variant="outlined"
              />
