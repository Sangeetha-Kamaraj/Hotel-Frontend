...
const BookingModal = ({ open, handleClose, selectedHotel = "" }) => {
  ...
  const makeBooking = async () => {
    const { selectedDate, selectedSeats, selectedSlot } = BookingState;
    const username = localStorage.getItem("username");
    ...
    if (selectedDate && selectedHotel && selectedSeats > 0 && selectedSlot && username) {
        try {
            ...
            console.log("Booking Response:", response); // Log the full response

            // Check the response data and status
            if (response.status === 201) {
                alert("Booking created successfully!");
                handleClose(); // Close the modal
            } else {
                alert("Booking failed: " + response.data); // Log any other response
            }
        } catch (error) {
